...
// Removed TypeScript interfaces (OrderItem) and type annotations. All state/data is plain JS.
...
export function OrderRevisionTab() {
  ...
  const handleQuantityOverride = (id, value) => {
    setOrderItems(
      orderItems.map((item) => {
        if (item.id === id) {
  ...
  const handlePriceOverride = (id, value) => {
    setOrderItems(
      orderItems.map((item) => {
        if (item.id === id) {
