...
export default function DietClerkPage() {
  const [mealCounts, setMealCounts] = useState<Record<string, Record<string, string>>>({})
  ...
  const handleMealCountChange = (diet: string, ward: string, value: string) => {
    setMealCounts((prev) => ({
      ...prev,
      [diet]: {
        ...prev[diet],
  ...
  const handleExtraItemChange = (item: string, value: string) => {
    setExtraItems((prev) => ({
      ...prev,
      [item]: value,
    }))
