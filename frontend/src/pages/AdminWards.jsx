...
const AdminWards = () => {
  ...
  const save = async () => {
    ...
    try {
      ...
      const payload = {
        code: editWard.code,
        name: editWard.name,
        beds: Number(editWard.beds) || 0,
        cots: Number(editWard.cots) || 0,
        icu: Number(editWard.icu) || 0,
        incubators: Number(editWard.incubators) || 0,
      };
  ...
  const deactivate = async (ward) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.message || "Failed to change ward status");
      }

      toast({
        title: "Ward Status Changed",
        description: `${ward.name} is now ${newStatus ? "Active" : "Inactive"}`,
      });
      ...
      toast({
        title: "Error",
        description: error.message || "Could not update ward status",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Dialog open={!!editWard} onOpenChange={() => setEditWard(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{isNew ? "Add Ward" : "Edit Ward"}</DialogTitle>
          </DialogHeader>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    value={editWard.code}
                    onChange={(e) =>
                      setEditWard({ ...editWard, code: e.target.value })
                    }
                  />
                </div>
                <div>
                  ...
                  <Input
                    value={editWard.name}
                    onChange={(e) =>
                      setEditWard({ ...editWard, name: e.target.value })
                    }
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setEditWard({
                        ...editWard,
                        cots: parseInt(e.target.value) || 0,
                      })
                    }
                  />
                </div>
                ...
                <div>
                  ...
                  <Input
                    type="number"
                    value={editWard.incubators}
                    onChange={(e) =>
                      setEditWard({
                        ...editWard,
                        incubators: parseInt(e.target.value) || 0,
                      })
                    }
                  />
                </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditWard(null)}>
              Cancel
            </Button>
